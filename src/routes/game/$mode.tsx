...
function GamePage() {
    ...
    // For free mode - word length (4-8) and random seed
    const getRandomLength = () => Math.floor(Math.random() * 5) + 4 // 4 to 8
    const [freeLength, setFreeLength] = useState(() => getRandomLength())
    ...
    const submitGuess = useCallback(async () => {
        ...
        try {
            ...
            // Clear animation after all tiles have flipped (100ms delay per tile + 500ms animation)
            setTimeout(
                () => {
            ...
            if (result.won) {
                ...
                if (
                    mode === 'suite' &&
                    suiteIndex < (dailyInfo?.suite.totalWords ?? 5) - 1
                ) {
                    ...
                    setGameOver(true)
                    const wordsFound =
                        mode === 'suite' ? (dailyInfo?.suite.totalWords ?? 5) : 1
                    if (playerId) {
                        updateStatsMutation({
                            odI: playerId,
                            name: playerName,
    ...
    const restart = () => {
        if (mode === 'free') {
            // Reset game state and get a new word with random length
            setFreeLength(getRandomLength())
        ...
        return (
            <div className="min-h-screen bg-linear-to-b from-slate-900 to-slate-800 flex flex-col">
                ...
                <main className="flex-1 flex flex-col items-center justify-center gap-6 p-4">
                    <Card className="bg-slate-800 max-w-sm w-full mx-4">
                        <CardContent className="pt-8 pb-8 text-center">
                            ...
                            <p className="text-gray-400 mb-6">
                                Vous avez déjà joué le {modeLabels[mode].toLowerCase()}{' '}
                                aujourd'hui.
                            </p>
                                ...
                                <div className="text-center">
                                    <div className="flex items-center justify-center gap-2 text-cyan-400 mb-1">
                                        ...
                                        <span className="font-mono text-xl">
                                            {formatTime(dailyCompletion.time || 0)}
                                        </span>
                                    </div>
